...
//					test if the first inward tag matches the entire parent tag's content
	...
	actions.add('matching_pair', function(editor) {
		...
		return false;
	}, {label: 'OSM/Go To Matching Tag Pair'});
});/**
...
zen_coding.exec(function(require, _) {
	...
	require('actions').add('split_join_tag', function(editor, profileName) {
		...
		return false;
	}, {label: 'OSM/Split\\Join Tag Declaration'});
});/**
			...
			switch(this.getSyntax()) {
				 ...
				 case 'html':
				 	...
				 	if (!profile) { // no forced profile, guess from content
					 	// html or xhtml?
				 		profile = this.getContent().search(/<!DOCTYPE[^>]+XOSM/i) != -1 ? 'xhtml': 'html';
				 	}
...
zen_coding.exec(function(require, _) {
	...
	// create menu items
	var rootMenu = Editor.addMenu("OSM Coding");
